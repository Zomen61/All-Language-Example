...
public class FileProcess {
	final private String[] FilterPhoto = {".jpg",".jepg",".png",".bmp"};
    ...
    String[] getFiles(File pPath,boolean pIncludeSubDirectory, String[]  pFilter,boolean overrideNowList){
        ...
        for( int i = 0; i < vFileArray.length; i++ )
        {
            ...
            if( pIncludeSubDirectory && f.isDirectory() ){
            	...
            	// 檔案, 則加入到 File List 中
            	for(String everyFilter:pFilter) {
                    if( everyFilter.equals("*") || f.getPath().indexOf(everyFilter) >= 0 ) {
	...
	Image openPhoto() {
		...
		if (tempFilePath.length() != 0) {
			...
			gFileList.clear();
			getFiles(movieDirectory,false,FilterPhoto,false);
